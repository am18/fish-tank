...
//
// This code is used to animate two creatures, modeled using OpenGL polyhedral transforms,
// and implemented using OpenGL display lists. The creatures are then contained inside a tank.
// The code below is repurposed from the original provided by Professor Bargal for the
// lab7 tank simulation. This code has kept the original naming conventions for the vivarium,
// tank and Fish classes and added classes Shark and Food. The class Shark has been
// been added as another creature in the tank, and has a predator-prey relationship
// with the Fish class. To determine incremental step of motion, I have used 
// potential function discussed in the Oct. 27 Stereo and collision lecture. Potential
// functions are also used to make the creature stay inside the tank and repel collisions
// with the walls. This has certain problems with avoiding the predator, which I have tried
// to fix by using higher significance levels for the predator-prey potential.
// Collision detection has been implemented through bounding spheres, which causes
// certain problems because the creatures are ellipsoids. I plan to fix this problem
// by either using bounding boxes or hierarchical bounding boxes.
// The user can interact with the system by using the mousing to rotate
// the tank in the 3D space. The user can also add food using keyboard key 'F', which calls
// class 'Food' and renders a solid sphere on the bottom of the tank. The fish moves toward
// food using an attractive potential function. Getting food has lower significance than 
// avoiding walls and the predator.
//****************************************************************************
